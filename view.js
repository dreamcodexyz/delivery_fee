...
window.OneStep.$(document).ready(function($){
    ...
    OneStep.Views.Shipping          = Backbone.View.extend({
        ...
        hdlChangeCountry: function(ev){
            ...
            if(onestepConfig.ajaxCountry){
                var params = {changebycountry: true, updateshippingaddress: false, updateshippingtype: (onestepConfig.ajaxShippingOnAddresss ? true : false), updatepaymenttype: true,updatepaymentmethod:true};
    ...
    OneStep.Views.Billing           = Backbone.View.extend({
        ...
        hdlChangeCountry: function(ev){
            ...
            if(onestepConfig.ajaxCountry){
                var params = {changebycountry: true, checkvat: (onestepConfig.validVAT ? true : false), updatebillingaddress: false, updateshippingtype: (onestepConfig.ajaxShippingOnAddresss ? true : false), updatepaymenttype: true,updatepaymentmethod:true};
                params.addition_post = {withoutaddressselect: true};
                view_onestep_init.update(params);
            }
            return true;
    ...
    OneStep.Views.Init              = Backbone.View.extend({
        ...
        initialize: function(){
            ...
            if(initLoad.both){
                ...
                this.update({updatepaymentmethod: true,updateshippingaddress:true,updateshippingtype:true, updateshippingmethod: true});
        ...
        hdlBtnCheckout: function(ev){
            ...
            if(window.OneStep.$('input[name=payment\\[method\\]]').filter(':checked').length == 0){
                if(!window.OneStep.$('#advice-required-entry_payment').length){
                    window.OneStep.$('#checkout-payment-method-load').append('<dt><div class="validation-advice" id="advice-required-entry_payment" style="">'+Translator.translate('Please select a payment method for your order!')+'</div></dt>');
                }
            }else{
            ...
            window.updateOnestep = window.OneStep.$.ajax({
                ...
                success :   function(data){
                    ...
                    if(typeof data.billing != 'undefined' && data.billing != ""){
                        ...
                        window.OneStep.$("#mw_onstepcheckout_billing_form").removeAttr('style');
                        window.OneStep.$("#billing-new-address-form").removeAttr('style');
                    }
                    if(typeof data.shipping != 'undefined' && data.shipping != ""){
                        window.OneStep.$("#mw_onstepcheckout_shipping_form").html(data.shipping);
                        window.OneStep.$("#mw_onstepcheckout_shipping_form").removeAttr('style')
                        window.OneStep.$("#shipping-new-address-form").removeAttr('style')
                    }
